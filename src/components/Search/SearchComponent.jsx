...
export default function SearchBarComponent({ onSearch }) {
  ...
  const handleChange = (e) => {
    ...
    if (searchMode === "name") {
      ...
      setSuggestions(
        mockIngredients.filter((ing) => ing.toLowerCase().includes(value.toLowerCase()))
      );
    }
  };
    ...
    <div className="search-bar-container">
      <div className="search-bar-wrapper">
        <input
          ...
          placeholder={searchMode === "name" ? "Search by recipe name..." : "Search by ingredient..."}
      ...
      {suggestions.length > 0 && (
        <div className="suggestions">
          {suggestions.map((s, i) => (
            <div key={i} className="suggestion-item" onClick={() => handleAdd(s)}>
              {s}
            </div>
          ))}
        </div>
